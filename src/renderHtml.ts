  ...
  return `
    ...
    <html lang="zh-CN">
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          .stat-card { position: relative; background: var(--card-bg); border: 1px solid var(--border); border-radius: 12px; padding: 28px 10px 10px 10px; box-shadow: 0 6px 14px var(--shadow); display: flex; flex-direction: column; align-items: center; justify-content: center; height: var(--stat-card-height); }
          .stat-label { position: absolute; top: 6px; left: 0; right: 0; text-align: center; color: var(--muted); font-size: 12px; }
          .stat-value { color: var(--text); font-size: 22px; font-weight: 600; text-align: center; }
          .stat-value div { text-align: center; }
          ...
          #dialog-edit .dialog { width: 360px; max-width: 360px; height: 500px; max-height: 500px; }
      ...
      <body>
        ...
        <div class="dialog-backdrop" id="confirm-edit">
          <div class="dialog">
            <div class="today-title" style="margin-bottom:8px;">确认修改</div>
            <div class="form-grid vertical" id="confirm-list"></div>
        ...
        <script>
          ...
          function computeSummaries(){ if(!DATA) return; var wear = 0, profit = 0, volume = 0; var range = getStatRange(); try { const now = new Date(); const pivot = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 8, 0, 0); const ref = now.getTime() < pivot.getTime() ? new Date(now.getFullYear(), now.getMonth(), now.getDate()-1) : new Date(now.getFullYear(), now.getMonth(), now.getDate()); Object.keys(DATA).forEach(k=>{ const ds = new Date(k+'T00:00:00'); const diffMs = ref.getTime() - ds.getTime(); const diffDays = Math.floor(diffMs / (24*60*60*1000)); if(range>0){ if(diffDays<0 || diffDays>range) return; } const entry = DATA[k]; const calcs = (entry && Array.isArray(entry.calc)) ? entry.calc : []; const uses = entry && Array.isArray(entry.use) ? entry.use : []; var dayVol = 0; var dayPts = 0; for(var ci=0; ci<calcs.length; ci++){ var c = calcs[ci]; var init = Number(c.initBalance)||0; var fin = Number(c.finalBalance)||0; wear += typeof c.wear!=='undefined' ? (Number(c.wear)||0) : (init - fin); var v = Number((c.txnVolume!==undefined?c.txnVolume: (c.volume!==undefined?c.volume: (c.transaction!==undefined?c.transaction: 0))))||0; if(v>0){ dayVol += v; } else if(typeof c.txnPts!=='undefined'){ var p = Number(c.txnPts)||0; if(p>0) dayPts += p; } }
            ...
            try { for(var i=0;i<uses.length;i++){ var u = uses[i]; var pg = Number(u.profitGross||0)||0; if(pg){ profit += pg; continue; } var up = Number(u.unitPrice||u.price||u.tokenVal||0)||0; var qty = Number(u.quantity||u.qty||u.tokenQty||0)||0; profit += up * qty; } } catch {}
          ...
          function clearEditInputs(){ var ids=['edit-init','edit-final','edit-vol','edit-txnpts','edit-balancepts','edit-wear','edit-time']; for(var i=0;i<ids.length;i++){ var el=document.getElementById(ids[i]); if(el) el.value=''; } }
          document.querySelectorAll('.stat-card').forEach(el=>{ el.addEventListener('dblclick', ()=>{ openDialogEdit(); clearEditInputs(); }); });
          function recalcEdit(){ var ef = document.getElementById('edit-final'); var ei = document.getElementById('edit-init'); var ev = document.getElementById('edit-vol'); var etp = document.getElementById('edit-txnpts'); var ebp = document.getElementById('edit-balancepts'); var ew = document.getElementById('edit-wear'); var init = Number((ei&&ei.value)||0)||0; var final = Number((ef&&ef.value)||0)||0; var vol = Number((ev&&ev.value)||0)||0; var txnpts = vol>=2? Math.floor(Math.log2(vol)) : Number((etp&&etp.value)||0)||0; var balpts = final>=100000 ? 4 : (final>=10000 ? 3 : (final>=1000 ? 2 : (final>=100 ? 1 : 0))); var wear = init - final; if(etp) etp.value = String(txnpts); if(ebp) ebp.value = String(balpts); if(ew) ew.value = String(wear); }
          ['edit-final','edit-init','edit-vol'].forEach(id=>{ var el = document.getElementById(id); if(el) el.addEventListener('input', recalcEdit); });
          let PENDING_EDIT = null;
          document.getElementById('edit-cancel').addEventListener('click',(e)=>{ e.preventDefault(); try { var cb = document.getElementById('confirm-edit'); if(cb) cb.style.display = 'none'; } catch {} closeDialogEdit(); });
          document.getElementById('edit-save').addEventListener('click', ()=>{ try { var ed = document.getElementById('edit-date'); var ds = ed? ed.textContent : ''; if(!ds) return; var fields = [ ['初始资金','orig-init','edit-init','initBalance'], ['完成资金','orig-final','edit-final','finalBalance'], ['交易量','orig-vol','edit-vol','transaction'], ['交易积分','orig-txnpts','edit-txnpts','txnPts'], ['持仓积分','orig-balancepts','edit-balancepts','balancePts'], ['磨损','orig-wear','edit-wear','wear'], ['时间','orig-time','edit-time','timestamp'] ]; var list = []; var payload = {}; for(var i=0;i<fields.length;i++){ var f = fields[i]; var o = document.getElementById(f[1]); var n = document.getElementById(f[2]); var ov = o? o.value: ''; var nv = n? n.value: ''; if(String(nv).trim()!==''){ list.push(f[0] + '：' + ov + ' → ' + nv); payload[f[3]] = nv; } } if(list.length===0){ alert('没有修改项'); return; } PENDING_EDIT = { ds, payload }; var cl = document.getElementById('confirm-list'); if(cl) cl.innerHTML = list.map(x=>'<div>'+x+'</div>').join(''); var cb = document.getElementById('confirm-edit'); if(cb) cb.style.display = 'flex'; } catch { alert('无法生成确认'); } });
          document.getElementById('confirm-cancel').addEventListener('click',()=>{ var cb = document.getElementById('confirm-edit'); if(cb) cb.style.display = 'none'; });
          document.getElementById('confirm-ok').addEventListener('click', async ()=>{ try { if(!PENDING_EDIT) return; var ds = PENDING_EDIT.ds; var p = PENDING_EDIT.payload; function numOrKeep(k){ var v = p[k]; if(k==='timestamp') return v; var n = Number(v); return Number.isFinite(n)? n : null; } var init = numOrKeep('initBalance'); var final = numOrKeep('finalBalance'); var vol = numOrKeep('transaction'); var txnpts = numOrKeep('txnPts'); var balpts = numOrKeep('balancePts'); var wear = numOrKeep('wear'); var time = p['timestamp']||''; if([init,final,vol,txnpts,balpts,wear].some(x=> x===null)){ alert('输入错误'); return; } if(!DATA) DATA = {}; if(!DATA[ds]) DATA[ds] = { calc: [], use: [] }; var calc0 = DATA[ds].calc && DATA[ds].calc[0]; if(!calc0){ calc0 = {}; DATA[ds].calc = [calc0]; } if(init!==undefined) calc0.initBalance = init; if(final!==undefined) calc0.finalBalance = final; if(txnpts!==undefined) calc0.txnPts = txnpts; if(balpts!==undefined) calc0.balancePts = balpts; if(wear!==undefined) calc0.wear = wear; if(time!==undefined) calc0.timestamp = time; if(vol!==undefined) calc0.transaction = vol; try { await fetch('/data', { method:'POST', headers:{ 'content-type':'application/json' }, body: JSON.stringify(DATA) }); } catch {} var cb = document.getElementById('confirm-edit'); if(cb) cb.style.display = 'none'; closeDialogEdit(); computeSummaries(); var td = document.getElementById('today-date'); if(td) fillStatsForDate(ds); PENDING_EDIT = null; } catch { alert('保存失败'); } });
          ...
          function computeEffectivePoints(){ if(!DATA) return; var sum = 0; try { Object.keys(DATA).forEach(k=>{ if(isValidDate(k)){ const entry = DATA[k]; const calc0 = entry && entry.calc && entry.calc[0]; if(calc0 && typeof calc0.txnPts !== 'undefined'){ sum += Number(calc0.txnPts)||0; } const uses = entry && entry.use; if(Array.isArray(uses)){ var deducted = 0; for(var i=0;i<uses.length;i++){ var u = uses[i]; var up = Number(u.usedPts||0)||0; deducted += up>0? up : 15; } sum -= deducted; } } }); } catch {} var el = document.getElementById('sum-valid'); if(el) el.textContent = String(sum); }
