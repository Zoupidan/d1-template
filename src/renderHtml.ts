    ...
    <html lang="zh-CN">
      ...
      <body>
        ...
        <script>
          ...
          function computeSummaries(){ if(!DATA) return; var wear = 0, profit = 0, volume = 0; var range = getStatRange(); try { const now = new Date(); const pivot = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 8, 0, 0); const ref = now.getTime() < pivot.getTime() ? new Date(now.getFullYear(), now.getMonth(), now.getDate()-1) : new Date(now.getFullYear(), now.getMonth(), now.getDate()); Object.keys(DATA).forEach(k=>{ const ds = new Date(k+'T00:00:00'); const diffMs = ref.getTime() - ds.getTime(); const diffDays = Math.floor(diffMs / (24*60*60*1000)); if(range>0){ if(diffDays<0 || diffDays>range) return; } const entry = DATA[k]; const calc0 = entry && entry.calc && entry.calc[0]; const uses = entry && Array.isArray(entry.use) ? entry.use : []; if(calc0){ var init = Number(calc0.initBalance)||0; var fin = Number(calc0.finalBalance)||0; wear += typeof calc0.wear!=='undefined' ? (Number(calc0.wear)||0) : (init - fin); var vol = Number((calc0.txnVolume!==undefined?calc0.txnVolume: (calc0.volume!==undefined?calc0.volume: 0)))||0; if(vol>0){ volume += vol; } else if(typeof calc0.txnPts!=='undefined'){ var pts = Number(calc0.txnPts)||0; if(pts>0) volume += pts; } }
