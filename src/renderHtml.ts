    ...
    <html lang="zh-CN">
      ...
      <body>
        ...
        <script>
          ...
          function openDialogCU(tab){ const backdrop = document.getElementById('dialog-cu'); backdrop.style.display = 'flex'; switchTabCU(tab); const now = currentTimestamp(); var label = document.getElementById('now-label'); if(label) label.textContent = '时间：' + now; var nc = document.getElementById('now-calc'); if(nc) nc.textContent = '时间：' + now; var nu = document.getElementById('now-use'); if(nu) nu.textContent = '时间：' + now; }
          ...
          function applyThemeFromControls(){ const cardOp = Number((document.getElementById('cfg-card-opacity')).value); const shOp = Number((document.getElementById('cfg-shadow-opacity')).value); const bcol = (document.getElementById('cfg-border-color')).value; const tcol = (document.getElementById('cfg-text-color')).value; const mcol = (document.getElementById('cfg-muted-color')).value; const btnBg = (document.getElementById('cfg-btn-bg')).value; const btnHover = (document.getElementById('cfg-btn-hover')).value; const sStart = (document.getElementById('cfg-sel-start')).value; const sEnd = (document.getElementById('cfg-sel-end')).value; const invalidBg = (document.getElementById('cfg-invalid-bg')).value; const root = document.documentElement; root.style.setProperty('--card-bg', 'rgba(' + [250,250,250].join(',') + ',' + (cardOp/100).toFixed(3) + ')'); root.style.setProperty('--shadow', 'rgba(' + [200,200,200].join(',') + ',' + (shOp/100).toFixed(3) + ')'); root.style.setProperty('--border', bcol); root.style.setProperty('--text', tcol); root.style.setProperty('--muted', mcol); root.style.setProperty('--button-bg', btnBg); root.style.setProperty('--button-hover-bg', btnHover); root.style.setProperty('--blue-start', sStart); root.style.setProperty('--blue', sEnd); root.style.setProperty('--invalid-bg', invalidBg); const themeObj = { cardOp, shOp, bcol, tcol, mcol, btnBg, btnHover, sStart, sEnd, invalidBg }; localStorage.setItem('alpha_theme', JSON.stringify(themeObj)); try { fetch('/theme', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(themeObj) }); } catch {} }
          ...
          function computeEffectivePoints(){ if(!DATA) return; var sum = 0; try { Object.keys(DATA).forEach(k=>{ if(isValidDate(k)){ const entry = DATA[k]; const calc0 = entry && entry.calc && entry.calc[0]; if(calc0 && typeof calc0.txnPts !== 'undefined'){ sum += Number(calc0.txnPts)||0; } } }); } catch {} var el = document.getElementById('sum-valid'); if(el) el.textContent = String(sum); }
          try { fetch('/data').then(r=> r.ok? r.json(): null).then(j=>{ DATA = j||null; buildMonth(state.y,state.m); computeEffectivePoints(); if(td) fillStatsForDate(td.textContent); }); } catch {}
        </script>
      </body>
    </html>
  `;
